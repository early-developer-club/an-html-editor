...
const SpacingInput = ({ label, value, onChange }: SpacingInputProps) => {
  ...
  useEffect(() => {
    if (value) {
      ...
      if (parts.length === 1) {
        setValues({
          top: strippedParts[0],
          right: strippedParts[0],
          bottom: strippedParts[0],
          left: strippedParts[0],
        })
        setMode('all')
      } else if (parts.length === 2) {
        setValues({
          top: strippedParts[0],
          right: strippedParts[1],
          bottom: strippedParts[0],
          left: strippedParts[1],
        })
        setMode('individual')
        ...
        setValues({
          top: strippedParts[0],
          right: strippedParts[1],
          bottom: strippedParts[2],
          left: strippedParts[3],
        })
        setMode('individual')
      }
    } else {
  ...
  const handleChange = (field: keyof typeof values, fieldValue: string) => {
    ...
    if (
      newValues.top === newValues.right &&
      newValues.top === newValues.bottom &&
      newValues.top === newValues.left
    ) {
      ...
      onChange(
        `${format(newValues.top)} ${format(newValues.right)} ${format(newValues.bottom)} ${format(newValues.left)}`
      )
    }
  }
  ...
  return (
    <div>
      <label className="block mt-3 mb-1 text-xs text-text-muted">{label}</label>
      <div className="flex items-center space-x-2">
        <button
          onClick={handleModeChange}
          className="p-1 text-xs rounded bg-button-bg text-text-primary"
        >
